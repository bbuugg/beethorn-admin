<template>
    <div class="container">
        <a-space class="container-action" size="large">
            <a-button v-action="'/resource/create'" type="primary" @click="goCreate">
                新建
            </a-button>
            <a-popconfirm
                v-action="'/resource/review'"
            ...
            >
                <a-button v-action="'/resource/review'" :disabled="selectedRowKeys.length > 0 ? false :true">
                    批量审核
                </a-button>
            </a-popconfirm>
            <a-button
                v-action="'/resource/recover'"
                v-if="status != 4"
                :disabled="selectedRowKeys.length > 0 ? false :true" 
        ...
        <span slot="action" slot-scope="text, record">
            <a @click="edit(record.id)" v-action="'/resource/edit'"  v-if="status != 4">编辑</a>
            <a @click="reduction(record.id)" v-action="'/resource/reduction'"   v-if="status == 4">还原</a>   
            <a-divider type="vertical" />
            <a @click="remove(record.id)" v-action="'/resource/remove'" v-if="status == 4">删除</a>   
            <a-dropdown v-if="status != 4">
                <a-menu slot="overlay">
                    <a-menu-item  v-action="'/resource/review'" v-if="record.status == 1">
                        <a-popconfirm
                            title="正在审核"
                            ok-text="通过"
                            cancel-text="拒绝"
                        ...
                        >
                            <a href="#" v-action="'/resource/review'">审核</a>
                        </a-popconfirm>
                    </a-menu-item>
                    <a-menu-item v-action="'/resource/recover'">
                        <a v-action="'/resource/recover'" @click="recover(record.id)">移入回收站</a>
                    </a-menu-item>
                </a-menu>
...
<script>
import { Remove,Review,Reduction,Recover } from '@/api/resource'
export default {
    props:{
        list:{
            type:Array,
    ...
    methods: {
        ...
        edit(e){
            this.$router.push({path: '/resource/edit',query:{id:e}});
        },
