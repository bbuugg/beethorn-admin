<template>
    <div class="container">
        ...
        <span slot="action" slot-scope="text, record">
            <a @click="edit(record.id)" v-action="'/article/edit'"  v-if="status != 4">编辑</a>
            <a @click="reduction(record.id)" v-action="'/article/reduction'"   v-if="status == 4">还原</a>   
            <a-divider type="vertical" />
            <a @click="remove(record.id)" v-action="'/article/remove'" v-if="status == 4">删除</a>   
            <a-dropdown v-if="status != 4">
                <a-menu slot="overlay">
                    <a-menu-item  v-action="'/article/review'" v-if="record.status == 1">
                        <a-popconfirm
                            title="正在审核"
                            ok-text="通过"
                            cancel-text="拒绝"
                        ...
                        >
                            <a href="#" v-action="'/article/review'">审核</a>
                        </a-popconfirm>
                    </a-menu-item>
                    <a-menu-item v-action="'/article/recover'">
                        <a v-action="'/article/recover'" @click="recover(record.id)">移入回收站</a>
                    </a-menu-item>
                </a-menu>
...
<script>
import { Remove,Review,Reduction,Recover } from '@/api/article'
export default {
    props:{
        list:{
            type:Array,
