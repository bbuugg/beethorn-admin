    ...
    <div class="container">
        <a-space class="container-action" size="large">
           <a-popconfirm
            ...
            >
                <a-button v-action="'/topic/review'" :disabled="selectedRowKeys.length > 0 ? false :true">
                    批量审核
                </a-button>
            </a-popconfirm>
            <a-popconfirm
        ...
        <span slot="action" slot-scope="text, record">
            <a-popconfirm
                 v-action="'/topic/top'"
                v-if="record.isTop == 1 && status != 4"
                title="修改帖子置顶状态"
                @confirm="top(2,record.id)"
            >
                <a href="#">置顶</a>
            </a-popconfirm>
            <a-popconfirm
                v-action="'/topic/top'"
                v-if="record.isTop == 2 && status != 4"
                title="修改帖子置顶状态"
                @confirm="top(1,record.id)"
            >
                <a href="#">取消置顶</a>
            </a-popconfirm>
            <a @click="reduction(record.id)"  v-action="'/topic/reduction'"  v-if="status == 4">还原</a>   
            <a-divider type="vertical" />
            <a @click="remove(record.id)"  v-action="'/topic/remove'"  v-if="status == 4">删除</a>   
            <a-dropdown v-if="status != 4">
                <a-menu slot="overlay">
                    <a-menu-item v-if="record.status == 1"  v-action="'/topic/review'">
                        <a-popconfirm
                            title="审核帖子状态"
                    ...
                    <a-menu-item v-action="'/topic/recover'">
...
<script>
import { Remove,Review,Top,Reduction,Recover } from '@/api/topic'
export default {
    props:{
        list:{
            type:Array,
