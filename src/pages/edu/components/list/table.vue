<template>
    <div class="container">
        <a-space class="container-action" size="large">
            <a-button v-action="'/edu/create'" type="primary" @click="goCreate">
                新建
            </a-button>
            <a-popconfirm
                v-action="'/edu/review'"
            ...
            >
                <a-button v-action="'/edu/review'" :disabled="selectedRowKeys.length > 0 ? false :true">
                    批量审核
                </a-button>
            </a-popconfirm>
            <a-button
                v-action="'/edu/recover'"
                v-if="status != 4"
                :disabled="selectedRowKeys.length > 0 ? false :true" 
        ...
        <span slot="action" slot-scope="text, record">
            <a @click="edit(record.id)" v-action="'/edu/edit'"  v-if="status != 4">编辑</a>
            <a @click="reduction(record.id)" v-action="'/edu/reduction'"   v-if="status == 4">还原</a>   
            <a-divider type="vertical" />
            <a @click="remove(record.id)" v-action="'/edu/remove'" v-if="status == 4">删除</a>   
            <a-dropdown v-if="status != 4">
                <a-menu slot="overlay">
                    <a-menu-item  v-action="'/edu/review'" v-if="record.status == 1">
                        <a-popconfirm
                            title="正在审核"
                            ok-text="通过"
                            cancel-text="拒绝"
                        ...
                        >
                            <a href="#" v-action="'/edu/review'">审核</a>
                        </a-popconfirm>
                    </a-menu-item>
                    <a-menu-item v-action="'/edu/recover'">
                        <a v-action="'/edu/recover'" @click="recover(record.id)">移入回收站</a>
                    </a-menu-item>
                    <a-menu-item>
                        <a @click="goJoin(record.id)">{{$t('view.join')}}</a>
                    </a-menu-item>
                </a-menu>
...
<script>
import { Remove,Review,Reduction,Recover } from '@/api/edu'
export default {
    props:{
        list:{
            type:Array,
        ...
        return {
            ...
            columns:[
                ...
                },
                {
                    title: "报名人数",
                    dataIndex: 'joins',
                    scopedSlots: { customRender: 'joins' }
                },
                {
                    title: "状态",
                    dataIndex: 'status',
    ...
    methods: {
        ...
        goCreate(){
            this.$router.push({path:'/edu/create'})
        },
        pageChange(e){
        ...
        edit(e){
            this.$router.push({path: '/edu/edit',query:{id:e}});
        },
        goJoin(e){
            this.$router.push({path: '/edu/joinList',query:{id:e}});
        },
