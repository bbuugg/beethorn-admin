<template>
    <div class="container">
        <a-space class="container-action" size="large">
            <a-button v-action="'/audio/create'" type="primary" @click="goCreate">
                新建
            </a-button>
            <a-popconfirm
                v-action="'/audio/review'"
            ...
            >
                <a-button v-action="'/audio/review'" :disabled="selectedRowKeys.length > 0 ? false :true">
                    批量审核
                </a-button>
            </a-popconfirm>
            <a-button
                v-action="'/audio/recover'"
                v-if="status != 4"
                :disabled="selectedRowKeys.length > 0 ? false :true" 
        ...
        <span slot="action" slot-scope="text, record">
            <a @click="edit(record.id)" v-action="'/audio/edit'"  v-if="status != 4">编辑</a>
            <a @click="reduction(record.id)" v-action="'/audio/reduction'"   v-if="status == 4">还原</a>   
            <a-divider type="vertical" />
            <a @click="remove(record.id)" v-action="'/audio/remove'" v-if="status == 4">删除</a>   
            <a-dropdown v-if="status != 4">
                <a-menu slot="overlay">
                    <a-menu-item  v-action="'/audio/review'" v-if="record.status == 1">
                        <a-popconfirm
                            title="正在审核"
                            ok-text="通过"
                            cancel-text="拒绝"
                        ...
                        >
                            <a href="#" v-action="'/audio/review'">审核</a>
                        </a-popconfirm>
                    </a-menu-item>
                    <a-menu-item v-action="'/audio/recover'">
                        <a v-action="'/audio/recover'" @click="recover(record.id)">移入回收站</a>
                    </a-menu-item>
                </a-menu>
...
<script>
import { Remove,Review,Reduction,Recover } from '@/api/video'
export default {
    props:{
        list:{
            type:Array,
    ...
    methods: {
        ...
        goCreate(){
            this.$router.push({path:'/video/create'})
        },
        pageChange(e){
        ...
        edit(e){
            this.$router.push({path: '/video/edit',query:{id:e}});
        },
